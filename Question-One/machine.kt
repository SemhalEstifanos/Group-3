...
data class Maintain(var name: String, var demand: String, var historicalData: Int, var personnel: Int)
fun maintenancePriority(machines: List<Maintain>) {
    for (item in machines) {
        if (item.demand == "High" && item.historicalData <= 3 && item.personnel <= 4) {
...
fun main() {
    val machines = listOf(
        Maintain("Machine A", "Low", 2, 4),
        Maintain("Machine B", "High", 3, 3),
    )
